...
class Transformer(nn.Module):
    ...
    # cam_feature
    def forward(self, x: torch.Tensor, cam_feature=None, attn_mask: Optional[torch.Tensor] = None):
        # pdb.set_trace()
        if not self.batch_first:
            x = x.transpose(0, 1).contiguous()  # NLD -> LND

        if cam_feature is not None:

            for i, r in enumerate(self.resblocks):
                if self.grad_checkpointing and not torch.jit.is_scripting():
                    # TODO: handle kwargs https://github.com/pytorch/pytorch/issues/79887#issuecomment-1161758372
                    clip_feat = checkpoint(r, clip_feat, None, None, attn_mask)
                else:
                    ...
                    clip_feat = x[:batch, :, :]
                    cam_feat = x[batch:, :, :]
                    cam_feat, clip_feat = self.CCGAs[i](clip_feat, cam_feat, cam_feature)
                    x = torch.cat((clip_feat, cam_feat), dim=0)
        else:
            for i, r in enumerate(self.resblocks):
                if self.grad_checkpointing and not torch.jit.is_scripting():
                    # TODO: handle kwargs https://github.com/pytorch/pytorch/issues/79887#issuecomment-1161758372
...
class VisionTransformer(nn.Module):
    ...
    # cam_feature
    def forward(self, x: torch.Tensor, cam_feature=None):
        # pdb.set_trace()
        ...
        # class embeddings and positional embeddings
        # x = torch.cat([_expand_token(self.class_embedding, x.shape[0]).to(x.dtype), x, _expand_token(self.seg_embedding, x.shape[0]).to(x.dtype)], dim=1)
        ...
        x = self.ln_pre(x)
        x = self.transformer(x, cam_feature)  # input : [bs,50,768] cam:[bs,64,96,96] -> [bs/2, 100, 768]
