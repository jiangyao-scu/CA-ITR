...
if __name__=='__main__':
    ...
    zoomnet = PvtV2B2_ZoomNeXt()
    zoomnet = zoomnet.to(device)

    num_params = 0
    for p in zoomnet.parameters():
        num_params += p.numel()
    print(zoomnet)
    print("The number of parameters: {}".format(num_params))

    input = torch.zeros((4, 3, 384, 384))
    input = input.to(device)
    GT = torch.ones((4, 1, 384, 384))
    GT = GT.to(device)

    data = {}
    data["image_l"] = F.interpolate(input, scale_factor=1.5, mode="bilinear", align_corners=False)
    data["image_m"] = F.interpolate(input, scale_factor=1.0, mode="bilinear", align_corners=False)
    data["image_s"] = F.interpolate(input, scale_factor=0.5, mode="bilinear", align_corners=False)
    data["mask"] = GT

    optimizer = torch.optim.Adam(zoomnet.parameters(), lr=5e-5, weight_decay=1e-8)

    coarse_pred, pred = zoomnet(data)

    # seg_loss = F.binary_cross_entropy_with_logits(pred, GT, reduction='mean')
